...
st.write(
    "Ask questions about Zyro Dynamics HR policies, benefits, leave, payroll, "
    "attendance, and employee guidelines."
)
...
# ----------------------------
# PDF locations
# "." means PDFs directly uploaded in GitHub main folder
# ----------------------------
CORPUS_PATHS = [
    ".",
    "./corpus",
...
def build_retriever():
    ...
    if pdf_root is None:
        raise FileNotFoundError(
            "No PDF files found. Upload HR policy PDFs to the main GitHub folder "
            "or place them inside a folder named 'corpus'."
        )
    ...
    if not documents:
        raise ValueError(
            "PDF files were found, but no readable text was loaded from them."
        )
...
def load_chains():
    ...
    return retriever, rag_chain, guardrail_chain


try:
    retriever, rag_chain, guardrail_chain = load_chains()
